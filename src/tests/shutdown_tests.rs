...
async fn test_task_system_idempotent_shutdown() {
    ...
    // Register a function
    task_system.register_function("test", task_function!(|x: i32| async move {
...
async fn test_concurrent_shutdown() {
    ...
    // Register a function
    task_system.register_function("sleep", task_function!(|ms: u64| async move {
...
async fn test_no_new_tasks_after_shutdown() {
    ...
    // Register a function
    task_system.register_function("test", task_function!(|x: i32| async move {
        Ok::<i32, crate::error::RustyRayError>(x)
    })).unwrap();
...
async fn test_task_completion_before_shutdown() {
    ...
    let slow_fn = move |args: Vec<Vec<u8>>| {
        ...
        Box::pin(async move {
            if args.len() != 1 {
                return Err(crate::error::RustyRayError::Internal("Expected 1 argument".to_string()));
            }
...
async fn test_shared_system_references() {
    ...
    ts_ref1.register_function("echo", task_function!(|x: String| async move {
        Ok::<String, crate::error::RustyRayError>(x)
    })).unwrap();
