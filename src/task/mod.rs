...
pub use object_ref::{ObjectRef, ObjectId};
pub use registry::{FunctionId, FunctionRegistry};
pub use spec::{TaskSpec, TaskType, TaskArg};
pub use manager::{TaskManager, TaskManagerConfig};
pub use system::{TaskSystem, TaskBuilder};
...
pub trait TaskReturn: Send + 'static {
    ...
    fn from_any(any: Box<dyn Any + Send>) -> Result<Self> where Self: Sized;
}
