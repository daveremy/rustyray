...
pub type TaskFunction = Arc<dyn Fn(Vec<Vec<u8>>) -> BoxFuture<'static, Result<Vec<u8>>> + Send + Sync>;
        ...
        F: Fn(Vec<Vec<u8>>) -> BoxFuture<'static, Result<Vec<u8>>> + Send + Sync + 'static,
    {
        if self.functions.contains_key(&id) {
            return Err(RustyRayError::Internal(
                format!("Function {} already registered", id)
            ));
        }
...
mod tests {
    ...
    async fn test_task_function_macro() {
        ...
        // Register it
        registry.register(FunctionId::from("multiply"), multiply_func).unwrap();
