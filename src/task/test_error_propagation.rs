...
mod tests {
    ...
    use crate::task::{TaskSystem, TaskBuilder};
    ...
    async fn test_task_error_propagation() -> Result<()> {
        ...
        task_system.register_function("failing_task", task_function!(|x: i32| async move {
            if x < 0 {
                Err(RustyRayError::Internal("Negative input not allowed".to_string()))
            } else {
                Err(RustyRayError::Internal("This task always fails".to_string()))
            }
        }))?;
        
        // Register a function that sometimes fails
        task_system.register_function("maybe_fail", task_function!(|x: i32| async move {
            if x < 0 {
        ...
        task_system.register_function("add_ten", task_function!(|x: i32| async move {
            Ok::<i32, RustyRayError>(x + 10)
        }))?;
    ...
    async fn test_error_with_multiple_args() -> Result<()> {
        ...
        task_system.register_function("validate_range", task_function!(|min: i32, value: i32, max: i32| async move {
            if value < min {
                Err(RustyRayError::Internal(format!("{} is less than minimum {}", value, min)))
            } else if value > max {
                Err(RustyRayError::Internal(format!("{} is greater than maximum {}", value, max)))
            } else {
