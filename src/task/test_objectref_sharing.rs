...
mod tests {
    ...
    async fn test_objectref_sharing_between_tasks() {
        ...
        // Register functions
        task_system.register_function("produce_data", task_function!(|| async move {
            Ok::<Vec<i32>, crate::error::RustyRayError>(vec![1, 2, 3, 4, 5])
        })).unwrap();
        
        task_system.register_function("sum", task_function!(|data: Vec<i32>| async move {
            Ok::<i32, crate::error::RustyRayError>(data.iter().sum())
        })).unwrap();
        
        task_system.register_function("max", task_function!(|data: Vec<i32>| async move {
            Ok::<i32, crate::error::RustyRayError>(*data.iter().max().unwrap_or(&0))
        })).unwrap();
        
        task_system.register_function("average", task_function!(|data: Vec<i32>| async move {
    ...
    async fn test_objectref_concurrent_access() {
        ...
        task_system.register_function("slow_compute", task_function!(|| async move {
