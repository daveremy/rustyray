...
async fn test_actor_to_actor_sharing() {
    ...
    with_test_runtime(|| async {
        ...
        let consumer = actor_system.create_actor(Consumer { processed_count: 0 }).await.unwrap();
        ...
        let consumer2 = actor_system.create_actor(Consumer { processed_count: 0 }).await.unwrap();
