...
impl Actor for DataProducer {
    async fn handle(&mut self, msg: Box<dyn Any + Send>) -> Result<Box<dyn Any + Send>> {
        if msg.downcast_ref::<String>().map(|s| s == "create").unwrap_or(false) {
            // Create some data
            let data = SharedData {
                id: self.id,
                values: vec![1, 2, 3, 4, 5].into_iter().map(|x| x * self.id as i32).collect(),
            };
...
impl Actor for DataConsumer {
    async fn handle(&mut self, msg: Box<dyn Any + Send>) -> Result<Box<dyn Any + Send>> {
        if let Some(obj_ref) = msg.downcast_ref::<rustyray_core::ObjectRef<SharedData>>() {
            ...
            Err(RustyRayError::Internal("Expected ObjectRef<SharedData>".to_string()))
        }
    }
}
...
async fn test_actor_creates_task_consumes() {
    ...
    // Create actor
    let producer = actor_system.create_actor(DataProducer { id: 10 }).await.unwrap();
    ...
    let task_ref: ObjectRef<Vec<i32>> = task_system
        .submit(TaskBuilder::new("process_data").arg_ref(&obj_ref).build().unwrap())
        .await.unwrap();
...
async fn test_shared_object_store() {
    ...
    task_system.register_function(
        "read_data_id",
        task_function!(|data: SharedData| async move {
            Ok::<u32, RustyRayError>(data.id)
        })
    ).unwrap();
    
    let task_ref: ObjectRef<u32> = task_system
        .submit(TaskBuilder::new("read_data_id").arg_ref(&obj_ref).build().unwrap())
        .await.unwrap();
        ...
        for (idx, obj_ref) in object_refs {
            ...
            let handle = tokio::spawn(async move {
                let task_ref: ObjectRef<i32> = task_system
                    .submit(TaskBuilder::new("sum_values").arg_ref(&obj_ref).build().unwrap())
                    .await
                    .unwrap();
