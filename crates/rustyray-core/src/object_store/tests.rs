...
async fn test_eviction_at_capacity() -> Result<()> {
    ...
    println!("Total size: {} bytes, capacity: {} bytes", stats.total_size, 1000);
    
    // With CLRU, strict enforcement means total size should never exceed capacity
    assert!(stats.total_size <= 1000, "Total size should not exceed capacity");
    ...
    assert!(stats_after.total_size <= 1000, "Total size still within capacity");
...
async fn test_single_object_exceeds_capacity() -> Result<()> {
    ...
    let test_size = bincode::serde::encode_to_vec(&large_data, bincode::config::standard())
        .unwrap()
        .len();
    println!("Actual serialized size: {} bytes (limit: 1000 bytes)", test_size);
...
async fn test_delete_unpin_race() -> Result<()> {
    ...
    assert!(!store.exists(id).await?, "Object should not exist after deletion");
...
async fn test_unpin_data_loss_scenario() -> Result<()> {
    ...
    let result = store.put(large_data.clone()).await;
    assert!(result.is_err(), "Should not be able to store object larger than cache");
    ...
    assert!(unpin_result.is_ok(), "Should be able to unpin when object fits");
...
async fn test_pinned_objects_blocking_capacity() -> Result<()> {
    ...
    assert!(stats.total_size <= 1000, "Total size should stay under capacity even with pinned objects");
