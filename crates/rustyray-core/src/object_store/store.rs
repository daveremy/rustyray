...
use super::types::{
    ObjectId, ObjectMetadata, PendingObject, PutResult, StoredObject, StoreConfig, StoreStats,
};
...
pub struct InMemoryStore {
    ...
    cache: Arc<Mutex<CLruCache<ObjectId, StoredObject, std::collections::hash_map::RandomState, ObjectWeight>>>,
...
impl InMemoryStore {
    ...
    pub fn new(config: StoreConfig) -> Self {
        ...
        let max_capacity = NonZeroUsize::new(config.max_memory).unwrap_or(NonZeroUsize::new(1).unwrap());
    ...
    fn serialize<T: Serialize>(object: &T) -> Result<Bytes> {
        ...
        crate::task::serde_utils::serialize(object)
...
impl ObjectStore for InMemoryStore {
    ...
    async fn unpin(&self, id: ObjectId) -> Result<()> {
        ...
        let entry = self.pinned.get(&id)
            .ok_or_else(|| RustyRayError::Internal(format!("Object {} not found or not pinned", id)))?;
        ...
        match result {
            Ok(evicted) => {
                ...
                if let Some((_, removed_entry)) = self.pinned.remove(&id) {
                    // Mark as unpinned
                    entry_clone.metadata.is_pinned.store(false, Ordering::Relaxed);
                    ...
                    self.stats.pinned_size.fetch_sub(removed_entry.metadata.size, Ordering::Relaxed);
                    Ok(())
                } else {
                    // Object was removed from pinned between our check and remove
                    // This is rare but possible - the object might have been deleted
                    Err(RustyRayError::Internal(format!("Object {} was removed during unpin", id)))
                }
            }
            Err((_id, _obj)) => {
    ...
    async fn delete(&self, id: ObjectId) -> Result<bool> {
        ...
        if let Some((_, entry)) = self.pinned.remove(&id) {
            ...
            self.stats.pinned_count.fetch_sub(1, Ordering::Relaxed);
            self.stats.pinned_size.fetch_sub(entry.metadata.size, Ordering::Relaxed);
            return Ok(true);
        }
