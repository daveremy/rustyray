...
enum TaskMessage {
    ...
    ObjectReady { id: ObjectId, data: Vec<u8> },
    ...
    pub fn with_timeout(registry: Arc<FunctionRegistry>, timeout: Duration) -> Self {
        let config = TaskManagerConfig {
            ...
            timeout_check_interval: Duration::from_millis(
                timeout.as_millis().min(1000) as u64 / 10,
            ), // 1/10th of timeout or 1 second
        };
...
impl TaskManagerWorker {
    ...
    async fn run(&mut self, mut rx: mpsc::Receiver<TaskMessage>) {
        ...
        while let Some(msg) = rx.recv().await {
            ...
            self.pending_tasks
                .retain(|task| !task.cancellation_token.is_cancelled());
        }
    ...
    async fn handle_submit(&mut self, spec: TaskSpec, result_tx: oneshot::Sender<Result<Vec<u8>>>, object_store: Arc<InMemoryStore>) {
        ...
        if deps.is_empty() {
            ...
            self.execute_task(spec, result_tx, cancellation_token, object_store);
        } else {
    ...
    async fn handle_object_ready(&mut self, id: ObjectId, data: Vec<u8>) {
        ...
        // Execute ready tasks
        for (spec, result_tx, cancellation_token, object_store) in ready_tasks {
            self.execute_task(spec, result_tx, cancellation_token, object_store);
        }
    }
    ...
    fn execute_task(
        ...
        result_tx: oneshot::Sender<Result<Vec<u8>>>,
        cancellation_token: CancellationToken,
        passed_object_store: Arc<InMemoryStore>,
    ) {
        let object_store = self.object_store.clone();
        let sender = self.sender.clone();
        let task_id = spec.task_id;
        let registry = self.registry.clone();
        let task_object_store = passed_object_store;
        ...
        let handle = tokio::spawn(async move {
            ...
            let execution = async {
                ...
                let resolved_args =
                    match Self::resolve_dependencies(&spec.args, &object_store).await {
                        Ok(args) => args,
                        Err(e) => {
                            eprintln!(
                                "Failed to resolve dependencies for task {}: {:?}",
                                spec.task_id, e
                            );
                            return Err(e);
                        }
                ...
                let function = match registry.get(&spec.function_id) {
                    Some(f) => f,
                    None => {
                        let error = RustyRayError::Internal(format!(
                            "Function {} not found",
                            spec.function_id
                        ));
                        eprintln!(
                            "Function {} not found for task {}",
                            spec.function_id, spec.task_id
                        );
                        return Err(error);
                    }
                };
...
mod tests {
    ...
    async fn test_simple_task_execution() {
        ...
        registry
            .register(func_id.clone(), |args, _context| {
                Box::pin(async move {
                    if args.len() != 2 {
