...
impl<T> ObjectRef<T> {
    ...
    pub(crate) fn new() -> (Self, oneshot::Sender<ObjectResult>) {
        ...
        let state = Arc::new(ObjectRefState { receiver: watch_rx });
...
impl<T: DeserializeOwned + Send + 'static> Future for ObjectRef<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let state = match &self.state {
            ...
            None => {
                return Poll::Ready(Err(RustyRayError::Internal(
                    "ObjectRef has no state - distributed execution not yet implemented"
                        .to_string(),
                )));
            }
        };
        ...
        if receiver.borrow().is_some() {
            ...
            match result {
                Ok(bytes) => {
                    let deserialized = crate::task::serde_utils::deserialize(&bytes).map_err(|e| {
                        RustyRayError::Internal(format!(
                            "Failed to deserialize object result: {}",
                            e
                        ))
                    });
...
mod tests {
    ...
    async fn test_object_ref_serialization() {
        ...
        // Deserialize
        let deserialized: ObjectRef<i32> =
            crate::task::serde_utils::deserialize(&serialized).unwrap();
