...
use rustyray::task::{TaskSystem, TaskBuilder, ObjectRef};
...
async fn main() -> Result<()> {
    ...
    // Square function
    task_system.register_function("square", task_function!(|x: i32| async move {
    ...
    // Add function
    task_system.register_function("add", task_function!(|x: i32, y: i32| async move {
        println!("   Task: Adding {} + {}", x, y);
        Ok::<i32, rustyray::error::RustyRayError>(x + y)
    }))?;
    
    // Multiply function
    task_system.register_function("multiply", task_function!(|x: i32, y: i32| async move {
