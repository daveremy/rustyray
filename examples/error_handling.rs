...
async fn main() -> Result<()> {
    ...
    task_system.register_function("divide", task_function!(|a: f64, b: f64| async move {
        if b == 0.0 {
    ...
    task_system.register_function("sqrt", task_function!(|x: f64| async move {
        if x < 0.0 {
            Err(RustyRayError::Internal(format!("Cannot take square root of negative number: {}", x)))
        } else {
    ...
    task_system.register_function("fetch_data", task_function!(|url: String| async move {
        if url.starts_with("http://") {
            Err(RustyRayError::Internal("Insecure protocol not allowed".to_string()))
    ...
    let tasks = vec![
        TaskBuilder::new("divide").arg(20.0).arg(4.0).submit::<f64>(&task_system).await?,
        TaskBuilder::new("divide").arg(15.0).arg(3.0).submit::<f64>(&task_system).await?,
        TaskBuilder::new("divide").arg(10.0).arg(0.0).submit::<f64>(&task_system).await?, // This will fail
        TaskBuilder::new("divide").arg(8.0).arg(2.0).submit::<f64>(&task_system).await?,
    ];
